...
`/` is the public landing page. Signed-in visitors are redirected off it to `/discover`,
which is where the old `/` app view now lives. `/login`, `/welcome`, `/dashboard`,
`/profile` and `/teachers/:id` are unchanged.
